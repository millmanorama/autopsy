...
public class PlasoIngestModule implements DataSourceIngestModule {
    ...
    private static final String PLASO_DB3 = "plasodb.db3";
    ...
    public ProcessResult process(Content dataSource, DataSourceIngestModuleProgress statusHelper) {
        ...
        Process log2TimeLine;
        try {            // Run log2timeline
            log2TimeLine = buildLog2TimeLineCommand(moduleOutputPath, image).start();
        ...
        try (BufferedReader log2TimeLineOutpout = new BufferedReader(new InputStreamReader(log2TimeLine.getInputStream()))) {
            //TODO: make l2tstatusprocessor implement thread directly.
        ...
        if (context.dataSourceIngestIsCancelled()) {
            logger.log(Level.INFO, Bundle.PlasoIngestModule_log2timeline_cancelled());  //TODO: de-internationalize log messages.
            MessageNotifyUtil.Message.info(Bundle.PlasoIngestModule_log2timeline_cancelled());
            return ProcessResult.OK;
        }

        //make sure plaso storage file was created.
        if (false == moduleOutputPath.resovle(PLASO).toFile().exists()) {
            logger.log(Level.INFO, Bundle.PlasoIngestModule_error_running_log2timeline());
        ...
        // 'sort' the output, really this just means output the plaso timeline data as a sqlite db.
        ExecUtil.execute(buildPsortCommand(moduleOutputPath), new DataSourceIngestModuleProcessTerminator(context));

        if (context.dataSourceIngestIsCancelled()) {
            logger.log(Level.INFO, Bundle.PlasoIngestModule_psort_cancelled());
            MessageNotifyUtil.Message.info(Bundle.PlasoIngestModule_psort_cancelled());
            return ProcessResult.OK;
        }

        File plasoFile = moduleOutputPath.resolve(PLASO_DB3).toFile();
        if (!plasoFile.exists()) {
    ...
    private ProcessBuilder buildPsortCommand(Path moduleOutputPath) {
        ProcessBuilder processBuilder = buildProcessWithRunAsInvoker("\"" + psortExecutable + "\"", //NON-NLS
                "-o", "4n6time_sqlite", //NON-NLS
                "-w",//NON-NLS
                moduleOutputPath.resolve(PLASO_DB3).toString(), //NON-NLS
                moduleOutputPath.resolve(PLASO).toString()
        );
    ...
    private void createPlasoArtifacts(String plasoDb, DataSourceIngestModuleProgress statusHelper) {
        ...
        String sqlStatement = "SELECT substr(filename,1) AS  filename, "
                              ...
                              + "   sourcetype "
                              + " FROM log2timeline "
                              + " WHERE source NOT IN ('FILE'," //duplicates what we already have 
                              + "       'WEBHIST')" // lots of bad dates, duplicates alot of what we already have from recent activity
                              + " AND sourcetype NOT IN ('PE Import Time'," // lots of bad dates
                              + "       'UNKNOWN');";  //NON-NLS
        ...
        try (SQLiteDBConnect tempdbconnect = new SQLiteDBConnect("org.sqlite.JDBC", "jdbc:sqlite:" + plasoDb); //NON-NLS
            ...
            while (resultSet.next()) {
                ...
                long eventType = findEventSubtype(source, sourceType);
    ...
    private AbstractFile getAbstractFile(String file) {
        ...
        // check the cached file //If we sort the results by file(name) will that reduce cache misses?
        ...
        try {
            ...
            abstractFiles.stream().filter((AbstractFile t) -> filePath.equalsIgnoreCase(t.getParentPath()))
    ...
    private long findEventSubtype(String plasoSource, String plasoSourceType) {
        //TODO: do we really need matches?  could we use a switch here form readability?

        if (plasoSource.matches("EVT")
            || plasoSource.matches("LOG")) {//NON-NLS
            return EventType.LOG_ENTRY.getTypeID();
        } else if (plasoSource.matches("REG")) {
    ...
    private static class L2TStatusProcessor implements Runnable, Cancellable {
        ...
        @Override
        public void run() {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputPath.resolve("log2timeline_output.txt").toFile()));) {//NON-NLS
                String line;
